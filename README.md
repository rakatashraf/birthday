...
GitHub Actions publishes `photo-collage` as a static site through GitHub Pages. The workflow runs automatically for pushes to `main` and can also be started manually from the Actions tab.

After GitHub Pages is enabled with **GitHub Actions** as its source, the site is available at `https://rakatashraf.github.io/birthday/`.
...
The automated checks cover the canonical GitHub Pages output, all asset references, the uploaded audio file, autoplay retry behavior, and the full slideshow and celebration loop.
